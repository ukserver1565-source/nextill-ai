...
import { Search, FileText, ChevronLeft, ChevronRight, Download, Trash2, Loader2, XCircle } from "lucide-react"
...
export default function DocumentsPage() {
  ...
  const handleDownload = async (doc: any) => {
    ...
    } catch (e: any) { setActionError(e.message || "Failed to download document") }
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed")
      fetchData()
    } catch (e: any) { setActionError(e.message || "Failed to delete document") }
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted mt-1">All documents created across the platform</p>
      </div>

      <div className="relative w-full max-w-xs">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted" />
        <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1) }} placeholder="Search documents..." className="w-full h-10 pl-10 pr-4 rounded-xl bg-card/80 border border-border text-foreground text-xs placeholder:text-muted/50 focus:outline-none focus:ring-2 focus:ring-[#6D5EF5]/30 transition-all" />
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left p-4 text-[11px] font-medium text-muted uppercase tracking-wider">Author</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="p-8 text-center text-xs text-muted">No documents found</td></tr>
              ) : (
                data.map((doc, i) => (
                  <motion.tr key={doc.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                    ...
                    <td className="p-4 text-xs text-foreground">{doc.profiles?.full_name || "—"}</td>
                    <td className="p-4 text-xs text-muted">{doc.tool_slug || "—"}</td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-xs text-muted">{doc.created_at ? new Date(doc.created_at).toLocaleDateString() : "—"}</td>
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => handleDownload(doc)} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-muted hover:text-foreground transition-all"><Download className="w-3.5 h-3.5" /></button>
                        <button onClick={() => handleDelete(doc.id)} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-muted hover:text-[#EF4444] transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </motion.tr>
                ))
