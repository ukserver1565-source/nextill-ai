...
export default function SecurityPage() {
  ...
  const severityStyles: Record<string, string> = {
    low: "bg-[#A7B0C0]/10 text-muted border-border",
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted mt-1">Security settings and event monitoring</p>
      </div>
      ...
      <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 rounded-xl bg-background border border-border">
              <div>
                ...
                <p className="text-xs text-muted mt-0.5">Protect against brute force attacks</p>
              </div>
            ...
            <div className="flex items-center justify-between p-4 rounded-xl bg-background border border-border">
              <div>
                ...
                <p className="text-xs text-muted mt-0.5">Enforce 2FA for admin accounts</p>
              </div>
            ...
            <div className="space-y-2 max-w-xs">
              <label className="text-xs font-medium text-muted">Session Timeout (minutes)</label>
              <div className="relative">
                <Clock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted" />
                <input type="number" value={sessionTimeout} onChange={(e) => setSessionTimeout(Number(e.target.value))} className="w-full h-11 pl-10 pr-4 rounded-xl bg-background border border-border text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-[#6D5EF5]/30 transition-all" />
              </div>
            </div>
          </div>
        ...
        <div className="liquid-glass-card border border-border rounded-xl p-6">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-sm font-medium text-muted mb-1">No security events recorded</p>
              <p className="text-xs text-muted">Events will appear here as they occur</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left py-3 pr-4 text-[11px] font-medium text-muted uppercase tracking-wider">Event</th>
                    <th className="text-left py-3 pr-4 text-[11px] font-medium text-muted uppercase tracking-wider">IP</th>
                    <th className="text-left py-3 pr-4 text-[11px] font-medium text-muted uppercase tracking-wider">Severity</th>
                    <th className="text-left py-3 pr-4 text-[11px] font-medium text-muted uppercase tracking-wider">Blocked</th>
                    <th className="text-left py-3 text-[11px] font-medium text-muted uppercase tracking-wider">Timestamp</th>
                  </tr>
                </thead>
                <tbody>
                  {securityLogs.map((event: any, i: number) => (
                    ...
                    >
                      <td className="py-3 pr-4 text-xs text-foreground">{event.event_type || event.event || "—"}</td>
                      <td className="py-3 pr-4 text-xs font-mono text-muted">{event.ip_address || event.ip_hash || "—"}</td>
                      ...
                      <td className="py-3 pr-4"><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-medium border ${event.blocked ? "bg-[#EF4444]/10 text-[#EF4444] border-[#EF4444]/20" : "bg-[#A7B0C0]/10 text-muted border-border"}`}>{event.blocked ? "Yes" : "No"}</span></td>
                      <td className="py-3 text-xs text-muted">{event.created_at ? new Date(event.created_at).toLocaleString() : "—"}</td>
                    </motion.tr>
                  ))}
                </tbody>
              </table>
