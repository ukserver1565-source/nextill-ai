    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted mt-1">User inquiries and support messages</p>
        </div>
      ...
      <div className="bg-card/80 backdrop-blur-xl border border-border rounded-xl overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center p-12">
            ...
            <span className="ml-3 text-sm text-muted">Loading messages...</span>
          </div>
        ) : error ? (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  <th className="text-left p-4 text-[11px] font-medium text-muted uppercase tracking-wider">From</th>
              ...
              <tbody>
                {messages.map((m, i) => (
                  <motion.tr
                    ...
                    className={`border-b border-border last:border-0 hover:bg-white/[0.02] transition-colors ${!m.read ? "bg-[#6D5EF5]/[0.02]" : ""}`}
                  >
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#6D5EF5]/20 to-[#8B5CF6]/20 border border-border flex items-center justify-center text-[10px] font-bold text-foreground">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs ${!m.read ? "text-foreground font-medium" : "text-muted"}`}>{m.subject}</span>
                      </div>
                      <p className="text-[10px] text-muted mt-0.5 truncate max-w-[250px]">{m.message}</p>
                    </td>
                    <td className="p-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-medium border ${
                        m.read ? "bg-[#A7B0C0]/10 text-muted border-border" : "bg-[#6D5EF5]/10 text-[#6D5EF5] border-[#6D5EF5]/20"
                      }`}>
                        {m.read ? "Read" : "New"}
                      </span>
                    </td>
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <a href={`mailto:${m.email}?subject=Re: ${encodeURIComponent(m.subject)}`} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-muted hover:text-[#4CC9F0] transition-all inline-flex"><Reply className="w-3.5 h-3.5" /></a>
                        <button onClick={() => handleMarkRead(m.id)} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-muted hover:text-foreground transition-all" aria-label="Mark as read"><CheckCheck className="w-3.5 h-3.5" /></button>
                        <button onClick={() => handleDelete(m.id)} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-muted hover:text-[#EF4444] transition-all" aria-label="Delete message"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </motion.tr>
                ))}
