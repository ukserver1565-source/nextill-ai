  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted mt-1">All user projects across the platform</p>
      </div>

      <div className="relative w-full max-w-xs">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted" />
        <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1) }} placeholder="Search projects..." className="w-full h-10 pl-10 pr-4 rounded-xl bg-card/80 border border-border text-foreground text-xs placeholder:text-muted/50 focus:outline-none focus:ring-2 focus:ring-[#6D5EF5]/30 transition-all" />
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left p-4 text-[11px] font-medium text-muted uppercase tracking-wider">Project</th>
                <th className="text-left p-4 text-[11px] font-medium text-muted uppercase tracking-wider">Owner</th>
                <th className="text-left p-4 text-[11px] font-medium text-muted uppercase tracking-wider">Documents</th>
                <th className="text-left p-4 text-[11px] font-medium text-muted uppercase tracking-wider">Created</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} className="p-8 text-center text-xs text-muted">No projects found</td></tr>
              ) : (
                data.map((proj, i) => (
                  <motion.tr key={proj.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#6D5EF5] to-[#8B5CF6] flex items-center justify-center text-sm font-bold text-foreground">
                          {(proj.name || "").charAt(0)}
                        </div>
                        <div>
                          <p className="text-sm font-medium text-foreground">{proj.name}</p>
                          <p className="text-[10px] text-muted">{proj.domain}</p>
                        </div>
                      </div>
                    </td>
