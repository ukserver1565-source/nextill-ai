...
const plans = [
  {
    ...
    credits: 50,
    popular: false,
    features: ["50 credits/month", "5 projects", "Basic tools access", "Standard support"],
  },
  {
    name: "Pro",
    price: "$29",
    credits: 500,
    popular: true,
    features: ["500 credits/month", "Unlimited projects", "All AI tools", "Priority support", "Advanced analytics", "API access"],
  },
  {
    name: "Enterprise",
    price: "$99",
    credits: 2000,
    popular: false,
    features: ["2000 credits/month", "Unlimited projects", "All AI tools", "Dedicated support", "Custom integrations", "Team accounts", "SLA guarantee"],
  },
]
    ...
    <div className="min-h-screen pt-20 sm:pt-24 pb-10 sm:pb-16 px-3 sm:px-4">
      <div className="w-full max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 max-w-lg sm:max-w-none mx-auto">
          {plans.map((plan) => (
            <div key={plan.name} className={`relative glass-card rounded-xl sm:rounded-2xl p-5 sm:p-6 ${plan.popular ? "border-primary-light/40 ring-1 ring-primary-light/20" : ""}`}>
              ...
              <p className="text-xs sm:text-sm text-muted mb-3 sm:mb-4">{plan.credits} credits per month</p>
              <Link href="/signup">
