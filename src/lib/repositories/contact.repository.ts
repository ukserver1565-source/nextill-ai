...
export const contactRepo = {
  async list(params: PaginationParams) {
    ...
    if (params.search) {
      query = query.or(`name.ilike.%${params.search}%,subject.ilike.%${params.search}%,email.ilike.%${params.search}%`)
    }
  ...
  async create(data: CreateContactMessageInput) {
    const { data: message, error } = await supabaseAdmin.from("contact_messages").insert(data).select().single()
    if (error) throw new Error(`Failed to create message: ${error.message}`)
    return message as ContactRow
  },
